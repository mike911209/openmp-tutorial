	...
	while ((conv > TOLERANCE) && (iters < MAX_ITERS))
	{
		...
		if ((iters & 1) == 0)
		{
			q.submit([&](sycl::handler &h)
			{
				...
				h.parallel_for(sycl::nd_range<1>(blocks * threads, threads), [=](sycl::nd_item<1> item) [[sycl::reqd_sub_group_size(64)]]
				{	
			...
			q.submit([&](sycl::handler &h)
			{
				...
				h.parallel_for(sycl::nd_range<1>(reduce_blocks_per_grid / 2, reduce_blocks_per_grid / 2), [=](sycl::nd_item<1> item) [[sycl::reqd_sub_group_size(64)]]
				{	
